...
#define FRAME_CORNER_RADIUS 12.5f

#define THUMB_GRADIENT_MAX_Y_WHITE 1.0f
#define THUMB_GRADIENT_MIN_Y_WHITE 0.9f
#define BACKGROUND_GRADIENT_MAX_Y_WHITE 0.2f
#define BACKGROUND_GRADIENT_MIN_Y_WHITE TWO_THIRDS
#define BACKGROUND_SHADOW_GRADIENT_WHITE 0.2f
#define BACKGROUND_SHADOW_GRADIENT_MAX_Y_ALPHA 0.2f
#define BACKGROUND_SHADOW_GRADIENT_MIN_Y_ALPHA 0.0f
#define BACKGROUND_SHADOW_GRADIENT_HEIGHT 20.0f
#define BORDER_WHITE 0.1f

#define THUMB_SHADOW_WHITE 1.0f
#define THUMB_SHADOW_ALPHA 0.6f
#define THUMB_SHADOW_BLUR 3.0f

#define DISABLED_OVERLAY_GRAY  1.0f
#define DISABLED_OVERLAY_ALPHA 0.0
...
- (void) furtherInit {
	...
	// NOTE(dk): start additions 
	self.showsOnOffLabels = NO;
	self.onOffSwitchControlColors = OnOffSwitchControlCustomColors;
	self.onSwitchLabel = @"ON";
  self.offSwitchLabel = @"OFF";
  self.customOffColor = [NSColor clearColor];
  self.customOnColor = [NSColor greenColor];
	// NOTE(dk): end additions
}
...
- (void)tintBackgroundWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
  ...
  //NSRect frameRect = NSMakeRect(cellFrame.origin.x+2, cellFrame.origin.y+2, cellFrame.size.width-4, cellFrame.size.height-4);
