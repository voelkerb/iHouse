...
#define VOICE_RESPONSES_ON @"The socket is turned on now.", @"Okay, socket on."
#define VOICE_COMMAND_OFF @"off"
#define VOICE_RESPONSES_OFF @"The socket is turned off now.", @"Okay, socket off."
...
NSString * const SocketSniffedSuccessfully = @"SocketSniffedSuccessfully";
NSString * const SocketSwitched = @"SocketSwitched";
...
- (void)sniffCMI {
  ...
  NSString *command = [NSString stringWithFormat:@"%@%@",COMMAND_SOCKET, COMMAND_SNIFF_CMI];
...
- (void)sniffReceived:(NSNotification *)notification {
  ...
  NSString *cmdPrefix = [NSString stringWithFormat:@"%@%@%@", COMMAND_RESPONSE, COMMAND_SOCKET, COMMAND_SNIFF_CMI];
  ...
  if ([[theCommand substringToIndex:[cmdPrefix length]] containsString:cmdPrefix]) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SocketSniffedSuccessfully object:nil];
  }
}
...
- (void)sniffCMIDismissed {
  ...
  NSString *command = [NSString stringWithFormat:@"%@%@",COMMAND_SOCKET, COMMAND_CMI_OFF];
  NSDictionary* commandDict = [NSDictionary dictionaryWithObjectsAndKeys:command , SerialConnectionHandlerCommand, nil];
  //[[NSNotificationCenter defaultCenter] postNotificationName:ConnectionHandlerSerialCommand object:commandDict];
  [connectionHandler sendSerialCommand:commandDict];
}
...
- (void)learnFreetec {
  ...
  freetecCode2 = arc4random_uniform(255);
  freetecCode3 = arc4random_uniform(5);
  // Generate the command to learn the freetec socket and send it with the connectionhandler
  NSData *data = [[NSData alloc] init];
  NSString *command = [NSString stringWithFormat:@"%@%@",COMMAND_SOCKET, COMMAND_FREETEC];
...
- (BOOL)toggle:(BOOL) theState {
  // Construct the command depending on the type of socket and on the state
  NSString *command = COMMAND_SOCKET;
  NSData *data = [[NSData alloc] init];
  switch (type) {
  ...
  if ([connectionHandler sendSerialCommand:commandDict]) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SocketSwitched object:nil];
    return true;
  }
  return false;
}
...
-(NSArray *)readableSelectors {
  return [[NSArray alloc] initWithObjects:@"Turn socket on", @"Turn socket off", @"Toggle socket", nil];
}
